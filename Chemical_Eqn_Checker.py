...
def mol_form(compound_formula):
    ...
    for i in range (len(compound_formula)):
        ...
        if compound_formula[i].isupper():
            name = compound_formula[i]
            
            if i== len(compound_formula) -1:
                atoms=1
            else:
                for j in range( i+1, len(compound_formula)):
                    if compound_formula[j].islower():
                        name+=compound_formula[j]
                    elif compound_formula[j].isdigit():
                        if atoms ==0:
                            atoms += int (compound_formula[j])
                        else:
                            atoms= atoms*10 +int(compound_formula[j])
                    else:
                        if atoms==0:
                            atoms=1
                            break
...
def expr_form(expr_coeffs,expr_molecs):
    ...
    for i in range(len(expr_molecs)):
            for key in expr_molecs[i]: #getting the key
                if key in dic:
                    dic[key] += expr_molecs[i][key] * expr_coeffs[i]
                else:
                    dic[key] = expr_molecs[i][key] * expr_coeffs[i]
    return dic    
...
def find_unbalanced_atoms(reactant_atoms, product_atoms):
    ...
    for key in reactant_atoms:
        if (key in product_atoms and reactant_atoms[key] == product_atoms[key]):
            uncom = set()
        elif (key in product_atoms and reactant_atoms[key] != product_atoms[key]):
            uncom.add(key)
        else:
            uncom = new_set2.symmetric_difference(new_set1)
...
def check_eqn_balance(reactants,products):
    ...
    #reactant
    open_reactant1= mol_form(reactants[1][0])
    open_reactant2= mol_form(reactants[1][1])
    new_tup1 = (open_reactant1,open_reactant2)
    val1 = expr_form(reactants[0],new_tup1)
    
    #product
    open_products1 = mol_form(products[1][0])
    open_products2 = mol_form(products[1][1])
    #print(open_products1,open_products2)
    new_tup2 = (open_products1,open_products2)
    val2 = expr_form(products[0],new_tup2)
   
    return find_unbalanced_atoms(val1, val2)
